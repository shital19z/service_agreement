...
# ===== NEW OPTIONS HANDLERS FOR BRANCH MANAGEMENT =====
...
def forgot_password(request: schemas.ForgotPasswordRequest, db: Session = Depends(get_db)):
    ...
    return JSONResponse(
        content={"message": "If your email exists, you will receive a link."},
        headers={
            "Access-Control-Allow-Origin": "http://localhost:5173",
            "Access-Control-Allow-Credentials": "true",
        }
...
def reset_password(request: ResetPasswordRequest, db: Session = Depends(get_db)):
    try:
        ...
        if token_data["used"]:
            ...
            return JSONResponse(
                status_code=400,
                content={"detail": "Token already used"},
                headers={
                    "Access-Control-Allow-Origin": "http://localhost:5173",
                    "Access-Control-Allow-Credentials": "true",
                }
...
def list_agreements(
    ...
    try:
        ...
        if current_user.role == "agent":
            ...
            agreements = db.query(models.Agreement).filter(
    ...
    try:
        ...
        if not branch:
            try:
                ...
                return JSONResponse(
                    status_code=400,
                    content={"detail": f"Invalid branch code: {agreement.branch_code}"},
                    headers={
                        "Access-Control-Allow-Origin": "http://localhost:5173",
                        "Access-Control-Allow-Credentials": "true",
                    }
        ...
        return JSONResponse(
            content={
                "id": new_agreement.id,
                "clt_first_name": new_agreement.clt_first_name,
                "clt_last_name": new_agreement.clt_last_name,
                "branch_code": new_agreement.branch_code,
                "status": new_agreement.status,
...
def get_agreement(
    ...
    try:
        ...
        agreement_dict = {
            ...
            "perc_charged": agreement.perc_charged,
...
def generate_share_link(
    ...
    try:
        ...
        return JSONResponse(
            content={"share_link": share_link, "token": token},
            headers={
                "Access-Control-Allow-Origin": "http://localhost:5173",
                "Access-Control-Allow-Credentials": "true",
            }
...
def share_via_email(
    ...
    try:
        ...
        if not email_sent:
            return JSONResponse(
                content={
                    "message": "Link generated but email delivery failed. You can copy the link manually.",
    ...
    try:
        ...
        for branch in branches:
            ...
            result.append({
                ...
                "tel": branch_phone,
                "fax": branch_fax,
                "mileage": branch.mileage or 0.67
            })
...
# ===== NEW BRANCH MANAGEMENT ENDPOINTS - UPDATED (No role restrictions) =====
...
def create_branch(
    ...
    """Create a new branch - Any logged-in user can create"""
    try:
        # REMOVED role check - any authenticated user can create
        ...
        if existing:
            return JSONResponse(
                status_code=400,
                content={"detail": f"Branch code '{branch.branch_code}' already exists"},
        ...
        new_branch = models.Branch(
            ...
            mileage=branch.mileage,
            admin_meds=branch.admin_meds,
        ...
        # Create default content for the new branch
        default_content = {
            ...
            "mileage_rate": branch.mileage or 0.67,
            "care_type": "Home Care",
        ...
        db.execute(
            ...
            """),
            {"code": branch.branch_code, "data": json.dumps(default_content)}
        )
        db.commit()
...
def update_branch(
    ...
    """Update an existing branch - Any logged-in user can update"""
    try:
        # REMOVED role check - any authenticated user can update
        
        existing = db.query(models.Branch).filter(
            models.Branch.branch_code == branch_code
        ).first()
        
        if not existing:
            return JSONResponse(
                status_code=404,
                content={"detail": f"Branch '{branch_code}' not found"},
                headers={"Access-Control-Allow-Origin": "http://localhost:5173"}
            )
        ...
        return JSONResponse(
            content={"message": "Branch updated successfully"},
            headers={"Access-Control-Allow-Origin": "http://localhost:5173"}
        )
...
def delete_branch(
    ...
    """Delete a branch - Any logged-in user can delete (if no agreements)"""
    try:
        # REMOVED role check - any authenticated user can delete
        
        agreements_count = db.query(models.Agreement).filter(
            models.Agreement.branch_code == branch_code
        ).count()
        ...
        if agreements_count > 0:
            return JSONResponse(
                status_code=400,
                content={
                    "detail": f"Cannot delete branch with {agreements_count} agreement(s). Please reassign or delete the agreements first."
                },
                headers={"Access-Control-Allow-Origin": "http://localhost:5173"}
            )
        
        # Delete branch content first
        db.execute(
            text("DELETE FROM branch_content WHERE branch_code = :code"),
            {"code": branch_code}
        )
        ...
        return JSONResponse(
            content={"message": "Branch deleted successfully"},
            headers={"Access-Control-Allow-Origin": "http://localhost:5173"}
        )
...
def get_branch_content(
    ...
    """Get content for a specific branch (for auto-populating agreement form)"""
    try:
        result = db.execute(
            text("""
                SELECT content_data FROM branch_content 
...
def update_branch_content(
    ...
    """Update branch content - Any logged-in user can update"""
    try:
        # REMOVED role check - any authenticated user can update content
        ...
        if existing:
            db.execute(
                text("""
                    ...
                    WHERE branch_code = :code AND content_type = 'agreement'
                """),
                {"code": branch_code, "data": json.dumps(content)}
            )
        else:
            db.execute(
                text("""
        ...
        return JSONResponse(
            content={"message": "Branch content updated successfully"},
            headers={"Access-Control-Allow-Origin": "http://localhost:5173"}
        )
...
def copy_branch_content(
    request: CopyContentRequest,
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Copy content from source branch to target branches"""
    try:
        results = {
            "agreements": 0,
            "rates": 0,
        ...
        # Get source branch content from branch_content table
        source_content = db.execute(
            text("SELECT content_data FROM branch_content WHERE branch_code = :code AND content_type = 'agreement'"),
            {"code": request.source_branch}
        ).first()
        
        if not source_content:
            return JSONResponse(
                status_code=404,
                content={"detail": f"Source branch '{request.source_branch}' has no content"},
                headers={"Access-Control-Allow-Origin": "http://localhost:5173"}
            )
        
        source_data = source_content[0]
        
        # Get source branch for rates (mileage, etc.)
        source_branch = db.query(models.Branch).filter(
            models.Branch.branch_code == request.source_branch
        ).first()
        
        for target_code in request.target_branches:
            target_branch = db.query(models.Branch).filter(
                models.Branch.branch_code == target_code
            ).first()
            
            if not target_branch:
                continue
            
            # Get existing target content or prepare for new
            existing = db.execute(
                text("SELECT content_data FROM branch_content WHERE branch_code = :code AND content_type = 'agreement'"),
                {"code": target_code}
            ).first()
            ...
            # ===== COPY AGREEMENT CONTENT (all text sections) =====
            if request.content_types.get("agreements", False):
                # List of all text content fields that should be copied
                text_fields = [
                    # Page 1 fields
                    "required_services",
                    "freq_of_visit",
                    ...
                    # General provisions (as JSON or text)
                    "general_provisions",
                    
                    # Page 2 fields - Rights and Procedures
                    ...
                    # Page 3.1 fields - Consumer Notice (if applicable)
                    "consumer_notice_text",
            ...
            if request.content_types.get("rates", False):
                # Copy mileage rate from branch table
                if source_branch:
                    target_branch.mileage = source_branch.mileage
                
                # Copy rate-related fields from content
                rate_fields = ["mileage_rate", "hourly_rate"]
            ...
            # Save the updated content back to database
            if existing:
                db.execute(
                    text("""
                        UPDATE branch_content 
                        ...
                        WHERE branch_code = :code AND content_type = 'agreement'
                    """),
                    {"code": target_code, "data": json.dumps(target_data)}
                )
            else:
                db.execute(
                    text("""
...
def get_branch_config_api(branch_code: str, state_code: str = "MD"):
    try:
        ...
        return JSONResponse(
            status_code=404,
            content={"detail": f"Branch config not found: {str(e)}"},
            headers={
                "Access-Control-Allow-Origin": "http://localhost:5173",
                "Access-Control-Allow-Credentials": "true",
            }
...
def get_shared_agreement(
    token: str,
    db: Session = Depends(get_db)
):
    try:
        shared = db.query(models.SharedAgreement).filter(
            models.SharedAgreement.token == token,
            models.SharedAgreement.is_active == True
        ).first()
        
        if not shared:
            return JSONResponse(
                status_code=404,
                content={"detail": "Invalid or expired share link"},
                headers={
                    "Access-Control-Allow-Origin": "http://localhost:5173",
                    "Access-Control-Allow-Credentials": "true",
                }
        ...
        agreement = db.query(models.Agreement).filter(
            models.Agreement.id == shared.agreement_id
        ).first()
...
def get_shared_agreement_pdf(
    ...
    try:
        ...
        pdf_data = {
            ...
            "handled_by": agreement.handled_by or "",
            "frequency_duration": agreement.frequency_duration or "",
            "care_type": agreement.care_type or "Home Care",
            "is_live_in": str(agreement.is_live_in).lower(),
            "hourly_rate": f"{float(agreement.hourly_rate):.2f}",
            "mileage_rate": f"{branch_mileage:.2f}",
            "vehicle_authorized": str(agreement.vehicle_authorized).lower(),
